...
class GramMatrix(nn.Module):
    def forward(self, input):
        # print(input.size())
        batch, c, w, h = input.size() 
        Gram = torch.bmm(input.reshape(batch, c, w * h), input.reshape(batch, w * h, c)) 
        return Gram.div(w * h)
...
class GramMSELoss(nn.Module):
    def forward(self, input, target):
        ...
        # print("end of GramMSELoss")
        return loss


class masked_loss(nn.Module):
    def __init__(self, person_mask, person_weight=15):
    ...
    def forward(self, input, target):
        ...
        person_mask = np.array(self.person_mask.resize(size, PIL.Image.BILINEAR)) / 255
        # skin = np.array(self.skin.resize(size, PIL.Image.BILINEAR)) / 255
        weight_map = np.ones((input.shape[2], input.shape[3]))
        weight_map[person_mask > 0.5] = self.person_weight
        # weight_map[skin > 0.5] = skin_weight
        weight_tensor = torch.from_numpy(weight_map).view(1,1,weight_map.shape[0], weight_map.shape[1])
        if torch.cuda.is_available():
        	weight_tensor.cuda()
        tmp = (input - target)**2 * weight_tensor
        loss = torch.mean(tmp)
        return loss
       
model_dir = 'url_image/back/style_models/'
...
def get_stylized_portret(style, portret, person_mask):
	...
	loss_fns = [GramMSELoss()] * len(style_layers) + [masked_loss(person_mask)] * len(content_layers)
	if torch.cuda.is_available():
